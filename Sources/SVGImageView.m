...
- (instancetype)initWithContentsOfURLandString:(NSURL *)url stringValue:(NSString *)string {
    if (self = [self init]) {
        [self _cr_loadSVGFromURLwithString:url stringValue:string];
    }
    return self;
}
...
- (void)_cr_loadSVGFromURLwithString:(NSURL *)url stringValue:(NSString *)string
{
#if defined(DEBUG) && !defined(POCKETSVG_DISABLE_FILEWATCH)
    if(_fileWatcher)
    ...
    _svgLayer.paths = [SVGBezierPath pathsFromSVGAtURLwithString:url stringValue:string];
}
